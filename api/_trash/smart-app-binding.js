...
class SmartAppBinding {
  ...
  static TRUSTED_ORIGINS = [
    // Production hosts — smdeltart.com (portal, studio, widgets)
    "https://smdeltart.com",
    "https://portal.smdeltart.com",
    "https://studio.smdeltart.com",
    "https://widgets.smdeltart.com",
    // caddeltai.com (media, CAD-AI pro)
    "https://caddeltai.com",
    "https://www.caddeltai.com",
    "https://media.caddeltai.com",
    "https://api.caddeltai.com",
    // topic.earth
    "https://topic.earth",
    "https://www.topic.earth",
    // Vercel production deployments (pattern matched in _isOriginTrusted)
    "https://smdeltart.vercel.app",
    // Development (localhost)
    "http://localhost:5500", // Widgets server
    "http://127.0.0.1:5500",
    "http://localhost:3000", // Studio server
    "http://127.0.0.1:3000",
    "http://localhost:8000", // Media-forge server
    "http://127.0.0.1:8000",
  ...
  // Vercel preview URL pattern (smdeltart-* and caddeltai-* staging deployments)
  static VERCEL_PREVIEW_PATTERN =
    /^https:\/\/(smdeltart|caddeltai)[a-z0-9-]*\.vercel\.app$/;

  // Authorized application fingerprints
  // Each app must be registered before it can access API keys.
  // Widgets now resolve on the shared widgets.smdeltart.com host via path routing.
  static AUTHORIZED_APPS = {
    ...
    "smart-iceoff": {
      name: "SmΔrt Off Ice",
      paths: ["/smart-iceoff.html", "/iceoff"],
      permissions: ["read", "write"],
      icon: "❄️",
      subdomain: "widgets.smdeltart.com",
    },
    "smart-editor": {
      name: "SmΔrt Editor",
      paths: ["/smart-editor.html", "/editor"],
      permissions: ["read", "write"],
      icon: "📝",
      subdomain: "widgets.smdeltart.com",
    },
    "smart-3d-editor": {
      name: "SmΔrt 3D Editor",
      paths: ["/smart-3d-editor.html", "/3d-editor"],
      permissions: ["read", "write"],
      icon: "🧊",
      subdomain: "widgets.smdeltart.com",
    },
  ...
  isAuthorized(permission = "read") {
    ...
    for (const [appId, app] of Object.entries(
      SmartAppBinding.AUTHORIZED_APPS,
    )) {
    ...
    if (!matchedApp) {
      ...
      console.warn(
        `⚠️ Path '${pathname}' not in authorized apps, but origin is trusted`,
      );
      matchedApp = {
        id: "unknown",
    ...
    if (
      !matchedApp.permissions.includes(permission) &&
      !matchedApp.permissions.includes("admin")
    ) {
      return {
        authorized: false,
  ...
  requestAccessToken(requester = "unknown") {
    ...
    sessionStorage.setItem(
      SmartAppBinding.ACCESS_TOKEN_KEY,
      JSON.stringify(token),
    );

    console.log(
      `🎫 Access token issued to ${token.appName} (expires in ${SmartAppBinding.TOKEN_EXPIRY_MS / 1000}s)`,
    );
  ...
  validateToken(token) {
    ...
    // Verify signature
    const expectedSignature = this._signToken({
      ...token,
      signature: undefined,
    });
  ...
  getProtectedSettings(token = null) {
    ...
    if (!validation.valid) {
      return {
        ...
        message:
          validation.message || `Token validation failed: ${validation.reason}`,
      };
    }
    ...
    try {
      ...
      if (encrypted) {
        ...
        return {
          success: true,
          settings: this._sanitizeSettings(
            settings,
            validation.token.permissions,
          ),
      ...
      if (plain) {
        ...
        return {
          ...
          warning:
            "Using unencrypted storage - consider upgrading to encrypted vault",
          app: validation.token.appName,
          tokenExpires: validation.remainingTime,
        };
      }
  ...
  _recordFailedAttempt() {
    this.failedAttempts++;
    console.warn(
      `⚠️ Failed authentication attempt ${this.failedAttempts}/${SmartAppBinding.MAX_FAILED_ATTEMPTS}`,
    );
    ...
    if (this.failedAttempts >= SmartAppBinding.MAX_FAILED_ATTEMPTS) {
      ...
      console.error(
        `🔒 Too many failed attempts! Locked out until ${new Date(this.lockoutUntil).toLocaleTimeString()}`,
      );
    }
  }
  ...
  _isOriginTrusted(origin) {
    ...
    if (
      SmartAppBinding.VERCEL_PREVIEW_PATTERN &&
  ...
  _saveAuthorizedApps() {
    ...
    for (const [id, app] of Object.entries(SmartAppBinding.AUTHORIZED_APPS)) {
      if (app.registeredAt) {
        // Only save custom apps
        apps[id] = app;
      }
    }
    localStorage.setItem(
      SmartAppBinding.AUTHORIZED_APPS_KEY,
      JSON.stringify(apps),
    );
  }
  ...
  _loadAuthorizedApps() {
    try {
      ...
      if (stored) {
        ...
        console.log(
          `📦 Loaded ${Object.keys(apps).length} custom authorized apps`,
        );
      }
    } catch (e) {
