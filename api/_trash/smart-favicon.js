...
(function () {
  ...
  function getSharedStopAt() {
    if (!window._smartAnimationStopAt) {
      ...
      console.log(
        `🎲 SmΔrt animation stopAt set to ${window._smartAnimationStopAt}/3`,
      );
    }
    ...
    if (rotation % 120 === 0) {
      ...
      if (thirdCount >= stopAt) {
        ...
        setTimeout(function () {
          ...
          updateFavicon();
          console.log(
            `✅ SmΔrt favicon stopped at ${thirdCount}/3 (color: ${["cyan", "green", "yellow"][colorIndex]})`,
          );
        }, 400);
      } else {
  ...
  function start() {
    ...
    // Static mode: skip animation, just set a fixed favicon URL
    if (window.SMART_FAVICON_STATIC) {
      const el = document.querySelector('link[rel="icon"]');
      if (el && typeof window.SMART_FAVICON_STATIC === "string") {
        el.href = window.SMART_FAVICON_STATIC;
      }
      return;
    }
