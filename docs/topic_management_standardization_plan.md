...
UX simplification rewrite, 2026-05-06: see `docs/topic_builder_lambda_user_simplification.md` for the newer Central Topic Composer issue. User testing showed that even `Describe -> Evidence -> Review` still feels like several tools; the next target is one smart input plus one editable posted-topic preview.

The goal is to stop treating `Smart Topic Builder`, `AI Research Assistant`, daily update candidates, source search, media generation, and admin export as separate tools. They should become one coherent workflow. The newest UI target is:

```text
One central input
  -> source/search/media/AI suggestions
  -> editable posted-topic preview
  -> save / export / submit / publish
```

The older architecture language can still describe the data path:
