...
const LOCAL_FIRST_ASSETS = new Map([
  ['assets/models/solar-system.glb', 'solar-system-readable-orbits-20260603'],
  ['assets/models/solar-system.real-orbits.linked.glb', 'solar-system-linked-moon-fix-20260605']
]);
...
export function resolveAssetUrl(path = '') {
  ...
  if (isLocalPreviewHost() && LOCAL_FIRST_ASSETS.has(normalized)) {
    const separator = rawPath.includes('?') ? '&' : '?';
    return `${rawPath}${separator}v=${LOCAL_FIRST_ASSETS.get(normalized)}`;
  }
