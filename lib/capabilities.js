...
const ADMIN_UNLOCK_STORAGE_KEY = 'euroearth_admin_unlocked';
...
function getConfiguredProfile() {
  ...
  if (getStoredAdminUnlock()) return 'admin';
...
function setStoredAdminUnlock(enabled) {
  try {
    localStorage.setItem(ADMIN_UNLOCK_STORAGE_KEY, enabled ? 'true' : 'false');
  } catch (error) {
    console.warn('[Access] Could not persist admin unlock:', error);
  }
}
...
export const AppAccess = {
  ...
  restoreLocalDevelopmentAdminMode() {
    return this.getState();
  },

  unlockAdminAccess(enabled = true) {
    setStoredAdminUnlock(Boolean(enabled));
    if (enabled) {
  ...
  setMode(mode) {
    ...
    const enabled = wantsAdmin && this.can('admin:ui-toggle');
  ...
  can(capability) {
    ...
    if (capability === 'admin:toggle' || capability === 'admin:ui-toggle') return adminProfile;
