...
import { DRACOLoader } from '../vendor/three/examples/jsm/loaders/DRACOLoader.mjs';
import { TIPPING_BOUNDARIES } from '../data/points.js';
import { resolveAssetUrl } from './asset-bridge.js?v=topic-earth-local-solar-mars-20260522-1813';
import { getTopicPreviewMediaUrl } from './media-utils.js';

const MODEL_ASSET_BASE = './assets/models';
const MAIN_TEXTURE_ASSET_BASE = './assets/textures/main';
const FEVER_TEXTURE_ASSET_BASE = './assets/textures/fever';
const DRACO_DECODER_PATH = './vendor/draco/';
const SOLAR_SYSTEM_MODEL_FILE = 'solar-system.real-orbits.linked.glb';
const dracoLoader = new DRACOLoader();
dracoLoader.setDecoderPath(DRACO_DECODER_PATH);

const assetPath = (base, fileName) => resolveAssetUrl(`${base}/${fileName}`);
const createModelLoader = () => {
  const loader = new GLTFLoader();
  loader.setDRACOLoader(dracoLoader);
  return loader;
};
const resolveAssetPath = (path, fallbackBase = '') => {
  if (!path) return null;
  if (/^(?:https?:)?\/\//.test(path)) {
    return path;
  }
  ...
  return fallbackBase ? assetPath(fallbackBase, path) : `./${path}`;
};

const SOLAR_SYSTEM_OBJECT_NAMES = [
  'Sun', 'Mercury', 'Venus', 'Earth', 'Moon', 'Mars', 'Jupiter', 'Saturn',
  'Titan', 'Enceladus', 'Uranus', 'Neptune', 'Pluto', 'Planet9', 'Planet 9',
  'Spaceship', 'StarShip', 'Asteroid', 'Asteroid2', 'Astroid', 'Astroid2',
  'Atlas31', 'Atlas32', 'Aura', 'OCO2', 'OCO-2', 'PACE'
];
const SOLAR_SYSTEM_OBJECT_SET = new Set(SOLAR_SYSTEM_OBJECT_NAMES);
const SOLAR_SYSTEM_OBJECT_ALIASES = {
  Astroid: 'Atlas31',
  Asteroid: 'Atlas31',
  Astroid2: 'Atlas32',
  Asteroid2: 'Atlas32',
...
const PLANET_FOCUS_TEXTURES = {
  Earth: {
    label: 'Earth 2025 Fever texture',
    flipY: true,
    variants: {
      '1k': 'assets/textures/fever/earth_2025_1k.png',
      '4k': 'assets/textures/fever/earth_2025_4k.png'
    }
  },
  Mercury: { label: 'Mercury high texture', map: 'assets/models/mercury/textures/Material.002_diffuse.jpeg' },
  Moon: { label: 'Moon high texture', map: 'assets/models/moon/textures/Material.002_diffuse.jpeg' },
  Mars: { label: 'Mars high texture', map: 'assets/models/mars/textures/lambert3_baseColor.jpeg' },
  Jupiter: {
    label: 'Jupiter high texture',
    map: 'assets/models/jupiter/textures/Planeta_baseColor.png',
    normal: 'assets/models/jupiter/textures/Planeta_normal.png'
  },
  Saturn: { label: 'Saturn high texture', map: 'assets/models/saturn/textures/Material.002_diffuse.jpeg' },
  Neptune: { label: 'Neptune high texture', map: 'assets/models/neptune/textures/neptune1_A_baseColor.png' },
  Pluto: { label: 'Pluto high texture', map: 'assets/models/pluton/textures/material_baseColor.jpeg' }
};
...
const EARTH_OBSERVATION_SATELLITES = [
  ...
  },
  {
    ...
    speed: 0.19,
    inclinationDeg: 66,
    ascendingNodeDeg: 252,
    phaseDeg: 278,
    color: 0xffd18a
  }
];

function stripGltfNameSuffix(name = '') {
  return String(name || '').replace(/\.\d+$/, '');
}

function canonicalSolarObjectName(name = '') {
  const stripped = stripGltfNameSuffix(name);
  const alias = SOLAR_SYSTEM_OBJECT_ALIASES[stripped] || stripped;
  if (SOLAR_SYSTEM_OBJECT_SET.has(alias)) return alias;
  const lower = alias.toLowerCase();
  const match = SOLAR_SYSTEM_OBJECT_NAMES.find(valid => lower.includes(valid.toLowerCase()));
  return match ? (SOLAR_SYSTEM_OBJECT_ALIASES[match] || match) : null;
}
...
export class GlobeRenderer {
  constructor(container, options = {}) {
    ...
    this.options = {
      earthTexture: options.earthTexture
        ? resolveAssetPath(options.earthTexture, MAIN_TEXTURE_ASSET_BASE)
        : assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_baseColor_1k.jpeg'),
    ...
    this.currentTopicMarkerId = null;
    this.currentTopicData = null;
    this.currentTopicTooltipVisible = false;
    this.currentTooltipAutoCloseTimer = null;
    this.tooltipPinned = false;
    this.tooltipContentKey = '';
    ...
    this.solarSystemFocusTextureCache = new Map();
    this.earthObservationSatellitePivots = [];
  ...
  initCamera() {
    ...
    this.camera.position.set(0, 0, this.options.initialDistance || 2.5);
  }
  ...
  async loadSolarSystem() {
    ...
    try {
      const gltf = await new Promise((resolve, reject) => {
        loader.load(
          assetPath(MODEL_ASSET_BASE, SOLAR_SYSTEM_MODEL_FILE),
          resolve,
          undefined,
          reject
        );
      ...
      this.installEarthObservationSatellites();
      ...
      this.solarSystemModel.traverse((child) => {
        if (canonicalSolarObjectName(child.name) === 'Earth') {
  ...
  findSolarSystemNodeByName(name) {
    if (!this.solarSystemModel) return null;
    const target = canonicalSolarObjectName(name);
    let match = null;
    this.solarSystemModel.traverse((child) => {
      if (match || !child.name) return;
      if (canonicalSolarObjectName(child.name) === target) {
        match = child;
      }
    });
    return match;
  ...
  installEarthObservationSatellites() {
    if (!this.solarSystemModel || this.earthObservationSatellitePivots.length) return;

    const earthOrbit = this.solarSystemModel.getObjectByName('Orbit_Earth') || this.findSolarSystemNodeByName('Earth')?.parent?.parent;
    if (!earthOrbit) {
      console.warn('[Space Satellites] Could not find Earth orbit pivot; satellite paths skipped.');
      return;
    }

    EARTH_OBSERVATION_SATELLITES.forEach((satellite) => {
      const pivot = new THREE.Group();
      pivot.name = `Orbit_${satellite.name}`;
      pivot.position.set(22, 0, 0);
      pivot.rotation.order = 'ZYX';
      pivot.rotation.z = THREE.MathUtils.degToRad(satellite.ascendingNodeDeg || 0);
      pivot.rotation.x = THREE.MathUtils.degToRad(satellite.inclinationDeg);
      pivot.userData.satelliteOrbitSpeed = satellite.speed;
      earthOrbit.add(pivot);

      const body = new THREE.Mesh(
        new THREE.SphereGeometry(satellite.size, 24, 16),
      ...
      body.name = satellite.name;
      const phase = THREE.MathUtils.degToRad(satellite.phaseDeg || 0);
      body.position.set(Math.cos(phase) * satellite.radius, Math.sin(phase) * satellite.radius, 0);
      body.userData.isEarthObservationSatellite = true;
      body.userData.displayName = satellite.label;
      pivot.add(body);

      const points = [];
      for (let index = 0; index < 144; index += 1) {
        const angle = (Math.PI * 2 * index) / 144;
        points.push(new THREE.Vector3(Math.cos(angle) * satellite.radius, Math.sin(angle) * satellite.radius, 0));
      }
      ...
      orbitLine.name = `OrbitPath_${satellite.name}`;
      pivot.add(orbitLine);

      this.earthObservationSatellitePivots.push(pivot);
    });
  }
  ...
  async loadAMOCOverlay() {
    ...
    try {
      const gltf = await new Promise((resolve, reject) => {
        loader.load(
          assetPath(MODEL_ASSET_BASE, 'amoc_circular_overlay.glb'),
          resolve,
          undefined,
          reject
        );
  ...
  async loadTippingOverlay() {
    ...
    try {
      const gltf = await new Promise((resolve, reject) => {
        loader.load(
          assetPath(MODEL_ASSET_BASE, 'tipping_point_circular.glb'),
          resolve,
          undefined,
          reject
        );
  ...
  async loadFeverTextures(quality = null) {
    ...
    const loadTexturePaths = async (year) => {
      const configuredPath = resolveAssetPath(textureSet[String(year)], FEVER_TEXTURE_ASSET_BASE);
      const legacyPath = resolveAssetPath(legacyTextureSet[String(year)], FEVER_TEXTURE_ASSET_BASE);
      const paths = [...new Set([
        configuredPath,
        assetPath(FEVER_TEXTURE_ASSET_BASE, `earth_${year}_${resolvedQuality}.png`),
        legacyPath,
        assetPath(FEVER_TEXTURE_ASSET_BASE, `earth_${year}.png`),
        resolvedQuality !== '1k' ? assetPath(FEVER_TEXTURE_ASSET_BASE, `earth_${year}_1k.png`) : null
      ].filter(Boolean))];
  ...
  getMainBaseColorTextureCandidates(quality) {
    ...
    const profiles = {
      '1k': [
        assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_baseColor_1k.jpeg'),
        assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_baseColor_1k.jpg'),
        assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_baseColor.jpeg')
      ],
      '4k': [
        assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_baseColor_4k.jpeg'),
        assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_baseColor_4k.jpg'),
        assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_baseColor.jpeg')
      ],
      '8k': [
        assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_baseColor_8k.jpeg'),
        assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_baseColor.jpeg')
      ]
    };
  ...
  getMainMaterialTexturePaths(quality) {
    ...
    const profiles = {
      '1k': {
        ...
        normal: assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_normal_1k.jpeg'),
        metallicRoughness: assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_metallicRoughness_1k.png')
      },
      '4k': {
        quality: '4k',
        baseColor: this.getMainBaseColorTexturePath('4k'),
        normal: assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_normal_4k.jpeg'),
        metallicRoughness: assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_metallicRoughness_4k.png')
      },
      '8k': {
        quality: '8k',
  ...
  createMainEarthMaterial(qualityOrPath = '1k') {
    const isDirectPath = typeof qualityOrPath === 'string'
      && (/^(?:https?:)?\/\//.test(qualityOrPath) || qualityOrPath.startsWith('./') || qualityOrPath.startsWith('/'));
    const texturePaths = isDirectPath
      ? {
          quality: 'direct',
          baseColor: resolveAssetPath(qualityOrPath, MAIN_TEXTURE_ASSET_BASE),
          normal: assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_normal_1k.jpeg'),
          metallicRoughness: assetPath(MAIN_TEXTURE_ASSET_BASE, 'Material.001_metallicRoughness_1k.png')
        }
  ...
  createCloudLayerCanvas(samples = []) {
    ...
    const getCloudTint = (sample = {}) => {
      const severity = String(sample.severity || '').toLowerCase();
      if (severity === 'severe') return { core: [255, 45, 85], mid: [255, 117, 99], edge: [255, 190, 130] };
      if (severity === 'warning') return { core: [255, 143, 0], mid: [255, 179, 64], edge: [255, 214, 125] };
      if (severity === 'notable') return { core: [255, 209, 102], mid: [255, 226, 150], edge: [255, 246, 205] };
      if (Number(sample.precipitation) > 0.05) return { core: [115, 207, 255], mid: [96, 174, 214], edge: [210, 242, 255] };
      return { core: [255, 255, 255], mid: [235, 245, 255], edge: [255, 255, 255] };
    };

    const rgba = ([r, g, b], alpha) => `rgba(${r}, ${g}, ${b}, ${alpha})`;

    const drawBlob = (x, y, radius, alpha, tint) => {
      const gradient = ctx.createRadialGradient(x, y, 0, x, y, radius);
      gradient.addColorStop(0, rgba(tint.core, alpha));
      gradient.addColorStop(0.45, rgba(tint.mid, alpha * 0.58));
    ...
    [...baseCloudCells, ...safeSamples].forEach((sample, index) => {
      ...
      const severityBoost = { severe: 0.24, warning: 0.18, notable: 0.1 }[String(sample.severity || '').toLowerCase()] || 0;
      const alpha = Math.min(0.94, 0.18 + cloud * 0.58 + Math.min(Number(sample.precipitation) || 0, 5) * 0.05 + severityBoost);
      const drift = ((index % 3) - 1) * 22;
      const tint = getCloudTint(sample);

      [-canvas.width, 0, canvas.width].forEach(offset => {
        drawBlob(x + offset, y, radius, alpha, tint);
        drawBlob(x + offset + drift, y + 18, radius * 0.62, alpha * 0.72, tint);
        drawBlob(x + offset - drift, y - 15, radius * 0.48, alpha * 0.55, tint);
      });
    });
  ...
  addMarker(point, color, isCluster = false) {
    ...
    const meteoSeverityColors = {
      severe: '#ff2d55',
      warning: '#ffb020',
      notable: '#ffd166',
      watch: '#4fc3f7'
    };
    const markerColor = point.markerColor || (isMeteoWarning
      ? (meteoSeverityColors[String(point.severity || '').toLowerCase()] || meteoSeverityColors.watch)
      : color);
    const visualSize = isCluster ? 0.018 : (isMeteoWarning ? 0.018 : (isRealtimeMeteo ? 0.014 : 0.008));
    ...
    if (isCluster) {
      ...
      marker.userData.glowBaseOpacity = glowMaterial.opacity;
    } else {
      const glowGeometry = new THREE.SphereGeometry(isMeteoWarning ? 0.034 : (isRealtimeMeteo ? 0.026 : 0.012), 8, 8);
      const glowMaterial = new THREE.MeshBasicMaterial({
        color: new THREE.Color(markerColor),
        transparent: true,
        opacity: isMeteoWarning ? 0.58 : (isRealtimeMeteo ? 0.42 : 0.2),
        blending: THREE.AdditiveBlending
      });
      const glow = new THREE.Mesh(glowGeometry, glowMaterial);
      glow.renderOrder = 5;
    ...
    marker.userData.visualBaseOpacity = material.opacity;
    marker.userData.isCurrentTopic = String(point?.id || '') === String(this.currentTopicMarkerId || '');
  ...
  focusOnPoint(lat, lon, options = {}) {
    ...
    const preferUpperViewport = Boolean(options.preferUpperViewport);
    const zoomDistance = Number(options.zoomDistance) || (preferUpperViewport ? 1.48 : 1.35);
    ...
    const verticalOffset = Number.isFinite(Number(options.verticalOffset))
      ...
      : (preferUpperViewport ? -0.34 : 0);

    if (verticalOffset) {
      endTarget.y += verticalOffset;
    }
  ...
  getPointDatePrefix(point = {}) {
    const dateValue = point.carbonHistory?.periodStart || point.date || point.year || '';
    ...
    return year ? year[1] : dateText;
  }

  getPointTitleWithDate(point = {}) {
    const title = String(point.title || 'Untitled topic').trim();
    const prefix = this.getPointDatePrefix(point);
    if (!prefix || title.startsWith(`${prefix}:`) || title.startsWith(`${prefix} -`)) {
      return title;
    }
    return `${prefix}: ${title}`;
  }

  getPointMediaUrl(point = {}) {
    return getTopicPreviewMediaUrl(point);
  }

  formatPointLocation(point = {}) {
    return [point.region, point.country].filter(Boolean).join(', ') || point.location || 'World topic';
  }

  getMarkerByPointId(pointId) {
    const id = String(pointId || '');
    if (!id) return null;
    return this.markers.find(marker => String(marker.userData?.id || '') === id) || null;
  }

  setCurrentTopic(point = {}) {
    const pointId = String(point?.id || '');
    this.currentTopicMarkerId = pointId || null;
    this.currentTopicData = pointId ? { ...point } : null;
    this.currentTopicTooltipVisible = Boolean(pointId);
    this.tooltipPinned = Boolean(pointId);

    this.markers.forEach(marker => {
      marker.userData.isCurrentTopic = pointId && String(marker.userData?.id || '') === pointId;
    });

    if (pointId) {
      this.updateCurrentTopicTooltipPosition(true);
      this.scheduleCurrentTopicTooltipClose();
    }
  }
  ...
  clearCurrentTopic() {
    ...
    this.currentTopicData = null;
    this.currentTopicTooltipVisible = false;
    this.clearCurrentTopicTooltipTimer();
    this.tooltipPinned = false;
    this.markers.forEach(marker => {
      marker.userData.isCurrentTopic = false;
      marker.scale.setScalar(1);
      if (marker.userData.glow) {
        marker.userData.glow.scale.setScalar(1);
        marker.userData.glow.material.opacity = marker.userData.glowBaseOpacity ?? 0.2;
      }
    });
    this.hideTooltip();
  }

  clearCurrentTopicTooltipTimer() {
    if (this.currentTooltipAutoCloseTimer) {
      clearTimeout(this.currentTooltipAutoCloseTimer);
      this.currentTooltipAutoCloseTimer = null;
    }
  }

  scheduleCurrentTopicTooltipClose(delay = 2000) {
    this.clearCurrentTopicTooltipTimer();
    this.currentTooltipAutoCloseTimer = setTimeout(() => {
      this.currentTopicTooltipVisible = false;
      this.tooltipPinned = false;
      this.tooltipContentKey = '';
  ...
  getMarkerScreenPosition(marker) {
    if (!marker || !this.camera) return null;
  ...
  updateCurrentTopicTooltipPosition(force = false) {
    if (!this.currentTopicMarkerId || !this.currentTopicData || this.inSolarSystemView) return;
    if (!force && !this.currentTopicTooltipVisible) return;
    const marker = this.getMarkerByPointId(this.currentTopicMarkerId);
    if (!marker || !marker.visible) {
      if (force) this.tooltip.classList.add('hidden');
      return;
    }

    const position = this.getMarkerScreenPosition(marker);
    if (!position) return;
    this.showTooltip(
      {
        ...this.currentTopicData,
        markerColor: marker.userData.markerColor || this.currentTopicData.markerColor
      },
      position.x,
      position.y,
      { pinned: true }
  ...
  positionTooltip(x, y, options = {}) {
    const pad = 12;
    const offset = options.pinned ? 18 : 15;
    const width = this.tooltip.offsetWidth || 300;
    const height = this.tooltip.offsetHeight || 110;
    const viewportWidth = window.innerWidth || this.container.clientWidth || 1024;
    const viewportHeight = window.innerHeight || this.container.clientHeight || 768;

    let left = x + offset;
    let top = y + offset;

    if (options.pinned && y > viewportHeight * 0.48) {
      top = y - height - offset;
    }

    left = Math.max(pad, Math.min(left, viewportWidth - width - pad));
    top = Math.max(82, Math.min(top, viewportHeight - height - pad));

    this.tooltip.style.left = `${left}px`;
    this.tooltip.style.top = `${top}px`;
  }

  showTooltip(pointData, x, y, options = {}) {
    if (
      this.tooltipPinned
      && !options.pinned
      && String(pointData?.id || '') !== String(this.currentTopicMarkerId || '')
    ) {
      return;
    }
    ...
    const categoryColors = {
      ...
      'climate': '#ff8a65',
      'extreme': '#ef5350',
      'carbon-history': '#f6c85f'
    };

    const color = pointData.markerColor || categoryColors[pointData.category] || '#00d4ff';
    const pinnedClass = options.pinned ? ' current-topic-tooltip' : '';
    const tooltipKindClass = pointData.isCluster ? ' cluster-globe-tooltip' : ' topic-globe-tooltip';
    ...
    if (pointData.isCluster) {
      contentKey = `cluster:${pointData.id || pointData.region || ''}:${pointData.count || ''}:${options.pinned ? 'pinned' : 'hover'}`;
      tooltipHtml = `
        <div class="tooltip-header" style="border-left: 3px solid ${this.escapeHtml(color)}">
          <div class="tooltip-title">${this.escapeHtml(pointData.count)} events in ${this.escapeHtml(pointData.region || 'this area')}</div>
          <div class="tooltip-meta">Click to view details</div>
        </div>
      `;
    } else {
      const mediaUrl = this.getPointMediaUrl(pointData);
      const title = this.getPointTitleWithDate(pointData);
      const location = this.formatPointLocation(pointData);
      const categoryLabel = this.formatCategoryLabel(pointData.category);
      contentKey = [
        'topic',
        pointData.id || '',
        title,
        ...
        mediaUrl,
        color,
        options.pinned ? 'pinned' : 'hover'
      ].join('|');
      const vignetteHtml = mediaUrl ? `
        <div class="tooltip-vignette-media">
          <img src="${this.escapeHtml(mediaUrl)}" alt="${this.escapeHtml(title)}" loading="lazy">
        </div>
      ` : `
        <div class="tooltip-vignette-media tooltip-vignette-generated" style="--tooltip-color: ${this.escapeHtml(color)}">
          <strong>${this.escapeHtml(categoryLabel)}</strong>
        </div>
      `;

      tooltipHtml = `
        <div class="tooltip-vignette">
          ${vignetteHtml}
          <div class="tooltip-header" style="border-left: 3px solid ${this.escapeHtml(color)}">
            <div class="tooltip-kicker">${this.escapeHtml(categoryLabel)}</div>
            <div class="tooltip-title">${this.escapeHtml(title)}</div>
            <div class="tooltip-meta">${this.escapeHtml(location)}</div>
          </div>
        </div>
      `;
    }
    
    if (this.tooltipContentKey !== contentKey) {
      this.tooltip.innerHTML = tooltipHtml;
      this.tooltipContentKey = contentKey;
    }

    const tooltipClassName = `globe-tooltip${tooltipKindClass}${pinnedClass}`;
    if (this.tooltip.className !== tooltipClassName) {
      this.tooltip.className = tooltipClassName;
    }
    this.tooltip.classList.remove('hidden');
    this.tooltipPinned = Boolean(options.pinned);
    this.positionTooltip(x, y, options);
  }

  hideTooltip(options = {}) {
    if (this.tooltipPinned && !options.force) return;
    if (options.force) {
      this.tooltipPinned = false;
      this.tooltipContentKey = '';
    }
  ...
  onMouseClick(event) {
    ...
    if (this.inSolarSystemView && this.solarSystemModel) {
      ...
      if (solarSystemIntersects.length > 0) {
        ...
        const isValidPlanet = Boolean(canonicalSolarObjectName(actualPlanet.name));
        
        if (!isValidPlanet) {
          // Not a valid planet - return to full solar system view
          this.focusedPlanet = null;
          this.focusedPlanetOffset = null;
          this.focusedPlanetSize = null;
          this.isPlanetFocused = false;
          return;
        }
  ...
  getPlanetInfo(planetObject) {
    ...
    const directName = canonicalSolarObjectName(planetObject.name);
    if (directName) {
      planetName = directName;
    } else {
      // Search up the hierarchy
      let current = planetObject;
      while (current && current !== this.solarSystemModel) {
        const parentName = canonicalSolarObjectName(current.name);
        if (parentName) {
    ...
    const planetData = {
      ...
      'Titan': {
        ...
        description: 'Saturn\'s largest moon, with a thick nitrogen atmosphere and methane lakes',
        diameter: '5,149 km',
        temperature: `About -179${degreeC}`,
        composition: 'Ice and rock with hydrocarbon lakes and organic chemistry',
        atmosphere: 'Dense nitrogen atmosphere with methane'
      },
      'Enceladus': {
        emoji: '\uD83E\uDDCA',
        type: 'Natural Satellite',
        description: 'A small icy moon of Saturn with active geysers and a subsurface ocean',
        diameter: '504 km',
        temperature: `About -201${degreeC}`,
        composition: 'Water ice shell over a global ocean and rocky core',
      ...
      'Planet9': {
        ...
        description: 'A hypothetical outer solar-system planet shown with a special compressed orbit',
        diameter: 'Unknown; commonly discussed as super-Earth/sub-Neptune scale',
        temperature: 'Unknown',
        composition: 'Unknown; hypothetical icy super-Earth or sub-Neptune-like body',
        atmosphere: 'Unknown'
      },
      'Planet 9': {
        emoji: '\uD83E\uDE90',
      ...
      'Aura': {
        ...
        description: 'NASA Aura observes atmospheric chemistry, including ozone, aerosols, and trace gases.',
        diameter: 'Spacecraft topic',
        temperature: 'Low Earth orbit thermal environment',
        composition: 'Aura spacecraft with atmospheric chemistry instruments',
        atmosphere: 'Studies ozone, air quality, aerosols, and climate-related trace gases',
        latestNews: 'Aura and OMI atmospheric monitoring updates'
      },
      'OCO2': {
        emoji: '\uD83D\uDEF0\uFE0F',
        type: 'Carbon Observatory',
        description: 'OCO-2 measures atmospheric carbon dioxide and plant fluorescence to track carbon-cycle signals.',
        diameter: 'Spacecraft topic',
        temperature: 'Low Earth orbit thermal environment',
        composition: 'Orbiting Carbon Observatory-2 spacecraft and spectrometers',
        atmosphere: 'Measures atmospheric carbon dioxide columns and vegetation health',
        latestNews: 'OCO-2 carbon and vegetation monitoring updates'
      },
      'PACE': {
        emoji: '\uD83D\uDEF0\uFE0F',
        type: 'Ocean-Atmosphere Observatory',
        description: 'PACE observes ocean color, aerosols, clouds, and atmosphere-ocean climate connections.',
        diameter: 'Spacecraft topic',
        temperature: 'Low Earth orbit thermal environment',
        composition: 'PACE spacecraft with OCI, SPEXone, and HARP2 instruments',
        atmosphere: 'Measures aerosols, clouds, air particles, and ocean-atmosphere interactions',
        latestNews: 'PACE ocean, cloud, and aerosol monitoring updates'
      }
    };
  ...
  resolveToPlanet(object) {
    ...
    if (canonicalSolarObjectName(object.name)) {
      return object;
    }
    ...
    let current = object;
    while (current && current !== this.solarSystemModel) {
      if (canonicalSolarObjectName(current.name)) {
        return current;
      }
      current = current.parent;
    }
  ...
  findSolarSystemObject(objectName) {
    if (!this.solarSystemModel || !objectName) return null;

    const aliases = {
      Spaceship: ['Spaceship', 'StarShip'],
      Asteroid: ['Asteroid', 'Astroid', 'Atlas31'],
      Asteroid2: ['Asteroid2', 'Astroid2', 'Atlas32'],
      OCO2: ['OCO2', 'OCO-2'],
      'OCO-2': ['OCO2', 'OCO-2']
    };
    const searchNames = aliases[objectName] || [objectName];
    const normalizedNames = searchNames.map(name => String(name).toLowerCase());
    let exactMatch = null;
    let fallbackMatch = null;

    this.solarSystemModel.traverse((child) => {
      if (!child.name || exactMatch) return;
      const childName = stripGltfNameSuffix(child.name).toLowerCase();
      if (normalizedNames.includes(childName)) {
        exactMatch = child;
        return;
      }
      if (!fallbackMatch && normalizedNames.some(name => childName.startsWith(name) || childName.includes(name))) {
        fallbackMatch = child;
      }
    });
  ...
  focusSolarSystemObject(objectName) {
    const object = this.findSolarSystemObject(objectName);
    if (!object) return null;

    const actualObject = this.resolveToPlanet(object);
    this.focusOnPlanet(actualObject);
    return this.getPlanetInfo(actualObject);
  }
  ...
  getPlanetFocusTexturePath(canonicalName) {
    const config = PLANET_FOCUS_TEXTURES[canonicalName];
    if (!config) return null;
    if (!config.variants) return config.map || null;

    const quality = window.innerWidth <= 768 || navigator.maxTouchPoints > 0 ? '1k' : '4k';
    return config.variants[quality] || config.variants['4k'] || config.variants['1k'] || null;
  }

  loadSolarSystemFocusTexture(path, label, options = {}) {
    const resolvedPath = resolveAssetPath(path, MODEL_ASSET_BASE);
    if (!resolvedPath) return null;
    const flipY = Boolean(options.flipY);
    const cacheKey = `${resolvedPath}|flipY:${flipY ? '1' : '0'}`;
    if (this.solarSystemFocusTextureCache.has(cacheKey)) {
      return this.solarSystemFocusTextureCache.get(cacheKey);
    }
    ...
    const texture = new THREE.TextureLoader().load(
      ...
      (loadedTexture) => {
        ...
        console.log(`[Space Texture] Loaded ${label}: ${resolvedPath}`);
      },
      undefined,
      (error) => console.warn(`[Space Texture] Could not load ${label}: ${resolvedPath}`, error)
    );
    texture.colorSpace = THREE.SRGBColorSpace;
    texture.flipY = flipY;
    this.solarSystemFocusTextureCache.set(cacheKey, texture);
    return texture;
  }

  applyPlanetFocusTexture(planetObject) {
    const canonicalName = canonicalSolarObjectName(planetObject?.name);
    if (!canonicalName) return;

    const config = PLANET_FOCUS_TEXTURES[canonicalName];
    const mapPath = this.getPlanetFocusTexturePath(canonicalName);
    if (!config || !mapPath) return;

    const map = this.loadSolarSystemFocusTexture(mapPath, config.label || canonicalName, {
      flipY: config.flipY
    });
    const normalMap = config.normal
      ? this.loadSolarSystemFocusTexture(config.normal, `${config.label || canonicalName} normal`, {
          flipY: config.normalFlipY
        })
      : null;
    if (!map) return;

    planetObject.traverse((child) => {
      if (!child.isMesh || !child.material || child.userData.spaceFocusTextureApplied === canonicalName) return;
      ...
      child.userData.spaceFocusTextureApplied = canonicalName;
    });
  }

  getPlanetFocusTarget(planetCenter, planetSize) {
    if (window.innerWidth > 768) return planetCenter;
    const cameraUp = new THREE.Vector3(0, 1, 0).applyQuaternion(this.camera.quaternion).normalize();
    const offset = Math.max(planetSize * 0.35, 0.55);
    return planetCenter.clone().add(cameraUp.multiplyScalar(-offset));
  }
  ...
  focusOnPlanet(planetObject) {
    ...
    this.applyPlanetFocusTexture(actualPlanet);
    ...
    const isEarthObservationSatellite = Boolean(actualPlanet.userData?.isEarthObservationSatellite);
    const distance = isEarthObservationSatellite ? 4.6 : Math.max(size * 4, 3);
    this.focusedPlanetSize = size;
    
    // Calculate camera offset from planet center
    const cameraOffset = isEarthObservationSatellite
      ? new THREE.Vector3(distance * 0.85, distance * 0.45, distance * 0.85)
      : new THREE.Vector3(distance * 0.7, distance * 0.3, distance * 0.7);
    ...
    const focusOffsetDistance = cameraOffset.length();
    const focusMinDistance = isEarthObservationSatellite
      ? 1.2
      : Math.max(0.8, Math.min(size * 0.85, focusOffsetDistance * 0.55));
    this.controls.minDistance = focusMinDistance;
    this.controls.maxDistance = Math.max(250, focusOffsetDistance * 8);
    this.controls.rotateSpeed = 1.0;
    this.controls.zoomSpeed = isEarthObservationSatellite ? 0.95 : 1.5;
    ...
    const animate = () => {
      ...
      const currentTarget = this.getPlanetFocusTarget(currentPlanetCenter, size);
      ...
      this.controls.target.lerpVectors(startTarget, currentTarget, eased);
      this.controls.update();
  ...
  applyFeverCameraPreset() {
    ...
    const targetPos = new THREE.Vector3(0, 0, smallScreen ? 4.15 : 3.5); // Wider framing
    const targetLookAt = new THREE.Vector3(0, 0, 0);
  ...
  animate() {
    ...
    // Update solar system animations. The GLB already compresses orbital time;
    // keep full Space mode lively, then slow down only when inspecting a planet.
    if (this.solarSystemMixer) {
      const speed = this.isPlanetFocused ? 0.12 : 0.35;
      this.solarSystemMixer.update(delta * speed);
    }

    if (this.earthObservationSatellitePivots?.length) {
      this.earthObservationSatellitePivots.forEach((pivot) => {
        pivot.rotation.z += (pivot.userData.satelliteOrbitSpeed || 0.1) * delta;
      });
    }
    ...
    if (this.focusedPlanet && this.focusedPlanetOffset && this.inSolarSystemView) {
      ...
      const planetTarget = this.getPlanetFocusTarget(planetCenter, this.focusedPlanetSize || 1);
      
      // Update controls target to planet center
      this.controls.target.copy(planetTarget);
    ...
    if (!this.inSolarSystemView) {
      this.markers.forEach(marker => {
        ...
        const phase = marker.userData.pulsePhase || 0;
        const isCurrentTopic = marker.userData.isCurrentTopic
          || (this.currentTopicMarkerId && String(marker.userData.id || '') === String(this.currentTopicMarkerId));

        if (isCurrentTopic) {
          const flash = Math.sin(time * 8 + phase) * 0.5 + 0.5;
          marker.scale.setScalar(1.06 + flash * 0.14);
          marker.material.opacity = Math.min(1, (marker.userData.visualBaseOpacity || 0.85) + 0.06);

          if (glow) {
            glow.scale.setScalar(1.12 + flash * 0.92);
            glow.material.opacity = 0.24 + flash * 0.22;
          }
        } else {
          marker.scale.setScalar(1);
          marker.material.opacity = marker.userData.visualBaseOpacity || marker.material.opacity;
          if (glow) {
            const pulse = Math.sin(time * 2 + phase) * 0.25 + 1.0;
            glow.scale.setScalar(pulse);
            glow.material.opacity = marker.userData.glowBaseOpacity ?? glow.material.opacity;
          }
        }
      });
    ...
    this.controls.update();
    this.updateCurrentTopicTooltipPosition();
