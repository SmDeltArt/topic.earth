...
export class LocalStorage {
  ...
  static compactTopicForStorage(topic = {}, index = 0) {
    ...
    const regionalState = this.compactRegionalState(topic.regionalState);
    if (regionalState) {
      compact.regionalState = regionalState;
    }
    ...
    if (topic.meteo && typeof topic.meteo === 'object') {
      compact.meteo = clonePlain(topic.meteo);
    }
  ...
  static compactRegionalState(state = null) {
    if (!state || typeof state !== 'object') return null;

    const normalizePoints = (points = [], limit = 1500) => (Array.isArray(points) ? points : [])
      .slice(0, limit)
      .map((point) => {
    ...
    const compact = {
      ...
      layerId: trimText(state.layerId || '', MAX_SHORT_TEXT_LENGTH),
      restoreMode: trimText(state.restoreMode || '', MAX_SHORT_TEXT_LENGTH),
      capturedAt: trimText(state.capturedAt || '', MAX_SHORT_TEXT_LENGTH)
    };

    if (state.focus && typeof state.focus === 'object') {
      compact.focus = {
        lat: Number(state.focus.lat),
        lon: Number(state.focus.lon),
        zoom: Number(state.focus.zoom),
        label: trimText(state.focus.label || '', MAX_SHORT_TEXT_LENGTH),
        precision: trimText(state.focus.precision || '', MAX_SHORT_TEXT_LENGTH)
      };
    }

    if (state.map && typeof state.map === 'object') {
      compact.map = {
        center: normalizePoints([state.map.center])[0] || null,
        zoom: Number(state.map.zoom),
        activeLayers: Array.isArray(state.map.activeLayers)
          ? state.map.activeLayers.slice(0, 40).map(layerId => trimText(layerId, MAX_SHORT_TEXT_LENGTH))
          : []
      };
    }

    if (state.path && typeof state.path === 'object') {
      const points = normalizePoints(state.path.points);
      if (points.length >= 2) {
        compact.path = {
          points,
          source: trimText(state.path.source || 'regional-map-drawn', MAX_SHORT_TEXT_LENGTH),
          label: trimText(state.path.label || '', MAX_SHORT_TEXT_LENGTH)
        };
      }
    }

    if (state.route && typeof state.route === 'object') {
      const waypoints = normalizePoints(state.route.waypoints || [], 12)
        .map(([lat, lon], index) => ({
          lat,
          lon,
          role: index === 0 ? 'start' : 'end'
        }));
      const geometry = normalizePoints(state.route.geometry);

      if (waypoints.length >= 2 || geometry.length >= 2) {
        compact.route = {
          profile: trimText(state.route.profile || 'bike', MAX_SHORT_TEXT_LENGTH),
          preference: trimText(state.route.preference || 'shortest', MAX_SHORT_TEXT_LENGTH),
          waypoints,
          geometry,
          distanceM: Math.max(0, Math.round(Number(state.route.distanceM) || 0)),
          durationS: Math.max(0, Math.round(Number(state.route.durationS) || 0)),
          provider: trimText(state.route.provider || '', MAX_SHORT_TEXT_LENGTH),
          fallback: Boolean(state.route.fallback),
          calculatedAt: trimText(state.route.calculatedAt || '', MAX_SHORT_TEXT_LENGTH)
        };
      }
    }

    if (!compact.path && !compact.route) return null;
  ...
  static compactMediaToken(input, topicId, index) {
    ...
    const compact = {
      ...
      mediaType: trimText(token.mediaType || '', MAX_SHORT_TEXT_LENGTH),
      embedUrl: this.compactUrl(token.embedUrl || '', topicId, `${tokenId}-embed`),
      thumbnailUrl: this.compactUrl(token.thumbnailUrl || '', topicId, `${tokenId}-thumb`),
      videoId: trimText(token.videoId || '', MAX_SHORT_TEXT_LENGTH),
      authorName: trimText(token.authorName || '', MAX_SHORT_TEXT_LENGTH),
    ...
    return (Array.isArray(sources) ? sources : [])
      ...
      .map((source, index) => {
        ...
        const compact = {
          ...
          provider: trimText(source.provider || '', MAX_SHORT_TEXT_LENGTH),
          authorName: trimText(source.authorName || '', MAX_SHORT_TEXT_LENGTH),
          adminNotes: trimText(source.adminNotes || '', MAX_SUMMARY_LENGTH),
          transcript: trimText(source.transcript || '', MAX_SUMMARY_LENGTH),
