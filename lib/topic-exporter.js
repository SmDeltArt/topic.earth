...
function normalizeStoryHtml(html = '', title = 'topic.earth story') {
...
function packageTopicStory(topic = {}, topicSlug, files, manifest) {
  const story = topic.topicStory && typeof topic.topicStory === 'object' ? topic.topicStory : null;
  if (!story?.enabled || !String(story.html || story.sanitizedHtml || '').trim()) return null;

  const storySlug = safeSegment(story.title || topic.title || 'story', 'story');
  const storyFolder = `story/${topicSlug}-${storySlug}`.slice(0, 150);
  const sanitizedHtml = String(story.sanitizedHtml || story.html || '').trim();
  const html = normalizeStoryHtml(sanitizedHtml, story.title || topic.title || 'topic.earth story');
  const storyManifest = {
    version: 1,
    type: story.type || story.style || 'story-card',
    style: story.style || story.type || 'story-card',
    title: story.title || topic.title || 'Topic story',
    ...
    structuredStory: story.structuredStory && typeof story.structuredStory === 'object' ? story.structuredStory : null
  };

  files.push({
    path: `${storyFolder}/index.html`,
    data: html
  });
  files.push({
    path: `${storyFolder}/story.json`,
    data: JSON.stringify(storyManifest, null, 2)
  });
  files.push({
    path: `${storyFolder}/source.html.txt`,
    data: String(story.html || '')
  });
  if (storyManifest.structuredStory) {
    files.push({
      path: `${storyFolder}/structured-story.json`,
      data: JSON.stringify(storyManifest.structuredStory, null, 2)
    });
  }
  ...
  const meta = {
    ...
    type: storyManifest.type,
    path: `${storyFolder}/index.html`,
    manifestPath: `${storyFolder}/story.json`,
    structuredPath: storyManifest.structuredStory ? `${storyFolder}/structured-story.json` : '',
    scriptsAllowed: false
  };
  manifest.storyCards.push(meta);
  return meta;
}
...
- story/: sandboxed HTML/SVG story cards attached to topics, plus bridge manifests when present
...
async function buildTopicPackage(topics, options = {}) {
  ...
  for (const topic of topics) {
    ...
    const storyMeta = packageTopicStory(topicCopy, topicSlug, files, manifest);

    topicCopy.media = packagedMedia;
    topicCopy.mediaAssets = mediaAssets;
    if (storyMeta) {
      topicCopy.topicStory = {
        ...(topicCopy.topicStory || {}),
        packagedPath: storyMeta.path,
        packagedManifestPath: storyMeta.manifestPath,
        scriptsAllowed: false
      };
    }
