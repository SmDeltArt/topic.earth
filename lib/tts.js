...
export class TTSManager {
  constructor(settings) {
    ...
    this.voices = [];
    this.recordedAudioByText = new Map();
    this.recordedAudioReady = false;
  ...
  async speak(text, langCode = 'en', options = {}) {
    ...
    const cachedUrls = this.getCachedAudioUrls(options, text, langCode);
  ...
  getCachedAudioUrls(options = {}, text = '', langCode = 'en') {
    ...
    const matched = this.getRecordedAudioUrlsForText(text, langCode);
    if (matched.length) {
      urls.unshift(...matched);
    }

    return [...new Set(urls.filter(Boolean).flatMap(url => this.expandAudioUrlCandidates(url)))];
  }

  async loadRecordedAudioIndex() {
    if (!window.fetch) return;

    try {
      const response = await fetch('./assets/audio/read-messages/tracking.json', { cache: 'force-cache' });
      if (!response.ok) return;
      const payload = await response.json();
      const messages = Array.isArray(payload?.messages) ? payload.messages : [];

      messages
        .filter(message => message?.webmExists && message?.text && message?.webm)
        .forEach(message => {
          const key = this.getRecordedAudioTextKey(message.text, message.lg || message.lang || 'en');
          if (!this.recordedAudioByText.has(key)) {
            this.recordedAudioByText.set(key, [
              `./assets/audio/read-messages/${message.webm}`,
              message.mp3 ? `./assets/audio/read-messages/${message.mp3}` : ''
            ].filter(Boolean));
          }
        });

      this.recordedAudioReady = true;
      console.info('[TTS] Recorded audio index loaded.', { count: this.recordedAudioByText.size });
    } catch (error) {
      console.warn('[TTS] Recorded audio index unavailable:', error?.message || error);
    }
  }

  getRecordedAudioTextKey(text = '', langCode = 'en') {
    const primary = this.getPrimaryLanguage(langCode || 'en');
    const normalizedText = String(text || '')
      .replace(/\s+/g, ' ')
      .replace(/\s+([.,;:!?])/g, '$1')
      .trim()
      .toLowerCase();
    return `${primary}|${normalizedText}`;
  }

  getRecordedAudioUrlsForText(text = '', langCode = 'en') {
    if (!this.recordedAudioReady || !text) return [];
    return this.recordedAudioByText.get(this.getRecordedAudioTextKey(text, langCode)) || [];
  }
